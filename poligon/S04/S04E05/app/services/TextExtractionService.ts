...
export class TextExtractionService {
    ...
    async extractTextFromImage(imageName: string): Promise<string> {
        ...
        const prompt = 'Extract all handwritten text from this image. Return only the text, without any commentary.';
        const messages = [
            {
                role: 'user' as const,
                content: [
        ...
        try {
            ...
            console.log('Received response from LLM service, text length:', text.length, 'characters');
            
            // Save the extracted text
            const outputPath = path.join(this.outputDir, 'extracted_text.txt');
