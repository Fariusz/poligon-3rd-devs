...
export class PdfService {
    ...
    constructor() {
        ...
        this.extractedTextPath = path.join(this.outputDir, 'extracted_text.txt');
    ...
    async extractTextFromPdf(pdfName: string, startPage: number = 1, endPage: number = 18): Promise<string> {
        ...
        if (fs.existsSync(this.extractedTextPath)) {
            console.log('Found existing extracted text file:', this.extractedTextPath);
            const text = fs.readFileSync(this.extractedTextPath, 'utf-8');
            console.log('Using existing extracted text (length:', text.length, 'characters)');
            return text;
        }
        ...
        try {
            ...
            console.log('PDF file read successfully, size:', (dataBuffer.length / 1024 / 1024).toFixed(2), 'MB');
            ...
            // Jeśli nie jest stringiem, wymuś konwersję na string przez JSON.stringify
            if (typeof extractedText !== 'string') {
                extractedText = JSON.stringify(extractedText, null, 2);
            }
            await this.saveExtractedText(extractedText, 'extracted_text.txt');
