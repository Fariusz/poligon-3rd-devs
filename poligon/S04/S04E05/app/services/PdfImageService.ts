...
export class PdfImageService {
    ...
    async convertPdfPageToImage(pdfName: string, pageNumber: number): Promise<string> {
        ...
        const gsCommand = `gswin64c -dSAFER -dBATCH -dNOPAUSE -sDEVICE=png16m -r200 -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -dFirstPage=${pageNumber} -dLastPage=${pageNumber} -sOutputFile="${outputPath}" "${pdfPath}"`;
