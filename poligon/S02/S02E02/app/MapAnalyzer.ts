...
export class MapAnalyzer {
  ...
  async run(): Promise<void> {
    try {
      ...
      // Load and compress fragments
      const fragments = await this.loadAndCompressFragments();
      ...
      for (let i = 0; i < fragments.length; i++) {
        const analysis = await this.visionAnalyzer.analyzeMapFragment(
          fragments[i],
          i + 1,
        );
      ...
      const instituteName =
        await this.cityIdentifier.identifyInstitute(fragmentAnalyses);
      ...
      if (instituteName && instituteName !== "NONE FOUND") {
        ...
        try {
          ...
          console.log(
            "Institute reporting failed, trying city identification...",
          );
        }
      }
      ...
      for (const taskName of possibleTaskNames) {
        ...
        try {
          ...
          if (taskName !== possibleTaskNames[0]) {
            await new Promise((resolve) => setTimeout(resolve, 2000));
          }
          await reportToCentrala({
            task: taskName,
          ...
          console.log(
            `Successfully reported to Centrala with task: ${taskName}!`,
          );
          return; // Success, exit
        } catch (error: any) {
          console.log(
            `Task name ${taskName} failed:`,
      ...
      // If all task names fail, try alternative analysis
      {
        ...
        const alternativeCityName =
          await this.cityIdentifier.identifyCityWithoutOutliers(
            fragmentAnalyses,
          );
        ...
        if (alternativeCityName !== cityName) {
          ...
          for (const taskName of possibleTaskNames) {
            try {
              ...
              console.log(
                `Successfully reported alternative answer with task: ${taskName}!`,
              );
              return;
  ...
  async loadAndCompressFragments(): Promise<Buffer[]> {
    const fragmentsDir = path.join(__dirname, "../fragments");
    const fragments: Buffer[] = [];

    try {
      console.log("Loading and compressing fragments...");

      // Load and compress fragments 1-4
      for (let i = 1; i <= 4; i++) {
        const fragmentPath = path.join(fragmentsDir, `fragment_${i}.jpg`);
        if (fs.existsSync(fragmentPath)) {
          const fragmentBuffer = fs.readFileSync(fragmentPath);
          const compressedBuffer =
            await this.imageProcessor.compressImageForAPI(fragmentBuffer);
          fragments.push(compressedBuffer);
          console.log(
            `Loaded and compressed fragment ${i} from: ${fragmentPath}`,
          );
          // Add delay between processing fragments
      ...
      throw new Error("Failed to process map fragments");
    }
  }
